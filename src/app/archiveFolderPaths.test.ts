...
describe("archive folder paths", () => {
  ...
  test("keeps a softly rounded open folder shape as the animation target", () => {
    expect(getArchiveFolderPaths(1)).toEqual({
      ...
      front:
        "M6.9 10.8h13.58c.82 0 1.42 .62 1.25 1.42l-1.15 4.58c-.24 1.02 -1.12 2.54 -2.38 2.62L18.2 19.42H5.72c-1.1 0 -1.82 -.8 -1.56 -1.78l1.15 -4.58c.26 -1.02 .9 -2.18 1.59 -2.26Z",
    });
  });
  ...
  test("interpolates coordinates between the closed and open folder shapes", () => {
    expect(getArchiveFolderPaths(0.5)).toEqual({
      ...
      front:
        "M6 10.4h13.69c.79 0 1.41 .63 1.32 1.41l-.57 5.24c-.12 1.18 -1.1 2.47 -2.39 2.51L18.05 19.56H5.91c-1.22 0 -2.11 -.94 -1.98 -2.09l.57 -5.24c.13 -.9 .77 -1.79 1.5 -1.83Z",
    });
  });
});

...
describe("LocalNotes", () => {
  ...
  test("adds a note and stores it in localStorage", async () => {
    const user = userEvent.setup();
    render(<LocalNotes initialIndex={0} />);

    await user.type(screen.getByLabelText("新便签内容"), "只给自己看的想法");
    await user.click(screen.getByRole("button", { name: "贴上去" }));
  ...
  test("submits a note with Enter", async () => {
    ...
    await user.type(screen.getByLabelText("新便签内容"), "回车贴上去{Enter}");
  ...
  test("keeps a newline with Shift Enter", async () => {
    ...
    await user.type(screen.getByLabelText("新便签内容"), "第一行{Shift>}{Enter}{/Shift}第二行");

    expect(screen.queryByText("第一行")).not.toBeInTheDocument();
    expect(screen.getByLabelText("新便签内容")).toHaveValue("第一行\n第二行");
  });
  ...
  test("uses the selected color when adding a note", async () => {
    ...
    await user.type(screen.getByLabelText("新便签内容"), "蓝色的新便签");
    await user.click(screen.getByRole("button", { name: "贴上去" }));

    expect(window.localStorage.getItem("sticky-notes.local-notes")).toContain('"tone":"blue"');
  });

  test("updates the compose note tone when selecting a color", async () => {
    const user = userEvent.setup();
    render(<LocalNotes initialIndex={0} />);
    ...
    expect(screen.getByRole("form", { name: "添加新便签" }).className).toContain("blue");
